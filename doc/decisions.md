...
## D-09 init 显式生成取代自动生成与全局层路径定稿

| 状态 | 日期 | 规范位置 |
|---|---|---|
| 已决策 | 2026-09-12 | design.md「零内置策略与默认配置生成（定稿）」+ 更正登记 23 |

**背景**：M2.6 定稿「三层皆缺自动生成项目侧默认包」（引导生成，`service.rs` 消费）；全局层发现路径 v1 留位未设计（`FoundLayers.global` 恒 None，D-07 词表 global 不可达）。P8/M8.1 用户逐项拍板。

**决策**：

- **移除自动生成**：任何模式不再静默写配置文件；`crush-tether init` 子命令成为配置创建唯一路径（缺省项目层 / `--user` / `--global`；`--engine` 选脚本后缀）。三层皆缺 → 引擎裸兜底（未匹配一律 confirm，`default=None` 既有语义）+ stderr 提示 init。
- **全局层路径定稿**：Unix `/etc/crush-tether/rules.toml`、Windows `%PROGRAMDATA%\crush-tether\rules.toml`；环境变量 `CRUSH_TETHER_GLOBAL_DIR` 优先（测试/便携覆盖，兼作集成测试隔离手段）。用户层维持 `~/.config/crush-tether/` 不变。
- 脚本层同构扩展：全局 → 用户 → 项目链（项目最后执行）；知识库 main 消费 v1 仍以项目层为准（多源合并见知识库节分期，A4）。
- [D-03](decisions.md#d-03-损坏重生成收窄)「损坏 ≠ 缺失」与「任一层有效即尊重现状」语义不变；init 对已存在文件一律不动（缺哪个补哪个）。

**依据**：安全门卫的配置应显式武装——静默落盘属惊扰面（用户不知情即改变裁决行为）；自动生成与零内置策略的显式化哲学不符，且全局层定型后「三层皆缺」需跨系统路径探测，隐式触发进一步失真。裸兜底 confirm 保证无配置时 fail-safe 性质不变（宁可全问、绝不放行）。

### 被否决的替代方案

> [!CAUTION] 【已否决】 保留 M2.6 自动生成 + 加开关
> 原因：默认值仍是「静默生成」，只把关闭责任推给用户；与「安全工具配置应显式武装」的拍板方向相反。
>
