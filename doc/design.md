...
- 解析优先级：**项目 > 用户 > 全局**（高层覆盖低层，不粘性）。`deny` 不被全局粘性锁定，均可被高层覆盖，属门卫而非沙箱。**三层同时存在时，效力顺序仍是 项目 > 用户 > 全局**；「三层皆缺」时引擎以裸兜底运行（未匹配一律 confirm）并 stderr 提示 init，默认配置**仅经 `crush-tether init` 显式生成**（见 [零内置策略与默认配置生成（定稿）](#零内置策略与默认配置生成定稿)、更正登记 23 与 [D-09](decisions.md#d-09-init-显式生成取代自动生成与全局层路径定稿)）。
...
用户级 `~/.config/crush-tether/`（维持不变）与项目同构。全局路径定稿（2026-09-12，P8/M8.1）：Unix `/etc/crush-tether/`、Windows `%PROGRAMDATA%\crush-tether\`；环境变量 `CRUSH_TETHER_GLOBAL_DIR` 优先（测试/便携覆盖）。脚本层**同文件按优先级**：全局 → 用户 → 项目，项目脚本最后执行，可作最终裁决。
...
- **生成仅经 `crush-tether init` 显式触发**（2026-09-12 更正：原「三层皆缺自动生成」已移除，见更正登记 23 与 [D-09](decisions.md#d-09-init-显式生成取代自动生成与全局层路径定稿)）：`init` 缺省生成项目层 `.crush-tether/`，`--user`/`--global` 切目标层，`--engine lua` 生成 `rules.lua`；**任何模式都不再静默写配置文件**——三层皆缺时引擎按裸兜底运行（未匹配一律 confirm，`default=None` 既有语义）并 stderr 提示 init；「任一层存在有效配置即尊重现状」语义不变。
- **损坏 ≠ 缺失（2026-09-06 收窄，语义不变）**：文件存在但解析失败 → stderr 告警 + 按 fail-safe confirm 兜底，**原文件不动、不留档**；init 对已存在的文件一律不动（缺哪个补哪个）。原「留档 `.bak-<时间戳>` 后重生成默认」方案的收窄论证见 [D-03](decisions.md#d-03-损坏重生成收窄)。
- **层位齐备**：三层默认包生成全部由 init 提供（v1 原只做项目层，P8/M8.1 补齐）；知识库 main 仍随包落目标层（v1 消费仍以项目层为准，多源合并见知识库节分期）。
- **引导豁免**：init 的生成动作不经规则链判定；只写目标层目录下固定文件名，不触碰其他路径。
- **幂等与原子**：模板内容恒定 → 重复 init 结果一致（已存在文件跳过）；落盘 temp + rename 原子替换，并发 init 天然收敛到同一结果。
- **fail-safe 衔接**：配置缺失 / init 失败按既有 fail-safe 处理（unparseable → confirm），绝不放行。
...
- **zcode**：hook 协议与 ClaudeCode 高度同构——模板变量 `${CLAUDE_PROJECT_DIR}`/`${ZCODE_PROJECT_DIR}` 双别名、`PreToolUse` 可返回 `allow`/`ask`/`deny` 三值决策（与三档一一映射）、exit 0/2 语义一致；支持 `type:"process"` 参数向量执行（Windows 免 shell 转义）。**实机探针定型（2026-09-06，M5.3）**：
  ...
  - **交付形态实测通过** = 插件分发（`hooks/hooks.json` 自动启用 hook runner；本地目录 marketplace 安装 + hook 实际触发全链验证）。配置文件 hook 轨（工作区 `.zcode/config.json` + `hooks.enabled:true`）实测未生效（疑因 `process` 型混入 `statusMessage` 字段被丢弃或需会话重启重载，未深究）——交付不依赖该轨，但登记为坑：`process` 型 hook 严格只接受 `command`/`args`/`timeoutMs`。**（更正 2026-09-08：未生效真因 = zcode 工作区 hook 审核门，statusMessage 疑因作废，见[更正登记](#更正登记对既有定稿) 21）**
...
- **源码/日志补充（2026-09-09，DeepWiki + v0.92.0 源码核对）**：聚合优先级全序 = halt > deny > allow；`updated_input` 取配置序最后一个提供者；`decision` 显式支持 `"none"`；hooks 视为可信用户配置、**不受 bash 工具黑名单（BlockFuncs）限制**；crushrc 另有第三注册途径 `hook add <event> --command CMD [--name] [--matcher] [--timeout]` builtin；上游已知 issue：#3482（payload 事件字段为 `event` 非 Claude 的 `hook_event_name`，单脚本按事件分支时静默 fall-through——本门探针按角色参数分支不受影响）、#3389（Windows `$HOME` 含反斜杠时 `~` 展开坑）。**headless 定性实锤**：`crush run` 下 `runner.Run` 未被调用（交互 TUI 5 条 `Hook completed` INFO vs run 零 hook 日志、零错误告警），三种注册途径（全局/项目 crush.json、crushrc `hook add`）均无效——判定 0.92.0 run 路径未接线（装配缺口），候选动作 = 提 crush issue 附复现步骤。**→ 2026-09-09 深夜二次更正（插桩排查，判定错误、「提 crush issue」作废）**：根因是我方 mock（`tmp/mock_llm.py`，现固化 `script/mock_llm.py`，OpenAI-compat 路径）返回的工具调用**缺必填 `description`**——fantasy 在工具分发前按 schema 校验，缺参**静默拒绝**（产出 error tool result，工具与 hook 从未执行，无日志）；而触发过 hooks 的 TUI 实验均为真实供应商模型驱动（参数自然完整）。「TUI 触发 / run 不触发」实为「真实模型 vs mock」混杂。修正 mock 一行后 `crush run -m mockspike` 全链触发（探针 dump + `Hook completed` INFO + 插桩 `hookedTool.Run` 实跑）；插桩另证 run 下接线全程健康（buildTools `pre_hooks=1` → wrap 26/26 → SetTools → 回合快照 26/26 全为 hookedTool）。**结论：`crush run` 正常执行 hooks，与 TUI 同链路，无装配缺口；编译版与 scoop 版同 commit（559ec80）同行为。**
...
- **分发两期分解（建议，待拍板）**：一期 = wrapper-only 插件（KB 级、零平台矩阵）+ PATH 二进制——不依赖捆绑即可杀掉 #2；二期 = 捆绑二进制（面向非 Rust 插件用户）+ scoop/Releases 管线。scoop manifest 机制上等价 GitHub Releases 路线（url 必须 http(s) 可取 + hash，无实用本地安装；本机 bucket 模板现成、仓库已 PUBLIC），触发条件 = 出现首个非 cargo 用户或对外宣传。**一期前提的实机验证已完成（2026-09-08，M7 前置）**：正式插件（`type:"process"` + 裸命令名 PATH 解析）在本机 zcode 实测三档裁决 / connect-or-spawn（serve 常驻、`mode:"serve"` 全程）/ JSONL 裁决日志全链通过——失效模式 #2 的「二进制可达」部署项在开发机闭环；安装启用机制实查：marketplace add 与安装走 UI（CLI `plugins` 子命令仅 list/enable），工作区 hooks 另有审核门（[更正登记](#更正登记对既有定稿) 21），均见 `cairn/plugin-distribution-analysis.md`。
- **wrapper 落地（P8/M8.2，2026-09-12）**：一期 wrapper-only 落地——`plugin/crush-tether/bin/` 增 sh+cmd 双 wrapper（保持哑：PATH 找到二进制 → 转发参数与 stdio/退出码；缺席 → exit 2 + 安装指引；永不解析信封），hooks.json `command` 改指 `${ZCODE_PLUGIN_ROOT}/bin/crush-tether-wrapper.cmd`，插件版本 0.1.0 → 0.2.0。**spike 定论（bundle 源码级，`zcode.cjs` 插桩核对）**：zcode 对 hook `command` 做 `${ZCODE_PLUGIN_ROOT}`/`${ZCODE_PLUGIN_ROOT}` 等模板变量展开——展开先于引号转义（路径含空格安全；官方 hookify 插件同款依赖）——wrapper 由插件根绝对路径拉起，不依赖 PATH，结构性补失效模式 #2。**平台坑**：批处理文件必须纯 ASCII——cmd.exe 按 OEM 代码页解析 batch，非 ASCII 注释被啃成杂命令（本机实测）；`type:"process"` 无 statusMessage 的三字段严格性不变。**边界与挂账**：hooks.json 单文件现指 .cmd，Linux zcode 公测后随 ubuntu job 换接 .sh（挂账同 zcode ubuntu 项）；CI 覆盖 = claude-ubuntu job 测 sh 版 / crush-win job 测 cmd 版（缺席+在场双路径）+ zcode-win job 经插件无人值守装配端到端实测（push 首跑验证）；本机 zcode 0.2.0 实弹随用户下次插件重装（现网 0.1.0 裸命令名行为不变，升级无感）。
...
- **`explain '<cmd>'`**：人读单发——逐条简单命令给出裁决 + 全溯源（命中层级/桶/token、归一链、写效果扫描集与逃逸判定、脚本改判、原因），组合裁决收尾；不落裁决日志（调试防噪音）。
- **`check --batch`**：stdin 一行一命令 → 裁决表（`ALLOW/CONFIRM/DENY` + 原因）；配置加载一次，exit 恒 0（表是报告不是阻断）。
- **`check --cases <file>`**：断言式规则用例批量对账（`version = 1` + `[[case]] cmd/expect`）；逐条 PASS/FAIL，全过 exit 0、任一失败 exit 1——规则变更的回归护栏。
- **`repl`**：用户面调试器（纯 stdio）。空行 = 重复上一条（覆盖「改规则 → 重跑同一命令」的复测主路径）；`!N` 重跑历史、`:hist`/`:lint`/`:q` 辅助。**每条输入重新加载配置**（项目配置毫秒级，天然「改规则即测、免重启」——不接 serve 的 notify 链）；不落裁决日志。

基建复用：`RuleSet::classify_single`（查表 → 脚本 → 定稿点的单命令共享原语；`decide_trace` 与 `explain` 同源——调试工具与门禁裁决恒一致）+ trace（M4.3）。与 M7.0 的关系：M7.0 验收四形态（读外纯读 / 读外写内 / 读内写外 / flag 写）以 `explain` 实跑验证（2026-09-08 闭环，写扫描集 `scan=[…]` 与 `ESCAPES` 判定直接可见）。

**存量配置迁移（M7.0 语义）**：`write_position` 只随**新生成**的默认包落地；已存在的项目配置不自动迁移（尊重现状原则，与「任一层有效即尊重」一致）——allow 桶含写型命令（touch/mkdir/cp/mv 等）的存量项目需自行补 `write_position = "last"` 条目，否则写逃逸防护缺失（读豁免语义下写外不再被拦）。
...
23. 「生成触发：三层合并仍得不到任何有效配置时，才在项目 `.crush-tether/` 写出默认包」（2026-09-04 定稿、M2.6 落地）→ **2026-09-12 移除自动生成**（用户拍板，P8/M8.1，[D-09](decisions.md#d-09-init-显式生成取代自动生成与全局层路径定稿)）：`init` 子命令成为配置创建唯一路径（缺省项目层 / `--user` / `--global`）；三层皆缺 → 引擎裸兜底（未匹配 confirm，`default=None` 既有语义）+ stderr 提示 init，不再静默写盘。理由：安全工具的配置应显式武装——静默生成属惊扰面，且全局层定型后「三层皆缺」需跨系统路径探测，自动生成的隐式性与门卫定位不符。同批定稿全局层路径：Unix `/etc/crush-tether/`、Windows `%PROGRAMDATA%\crush-tether\`、`CRUSH_TETHER_GLOBAL_DIR` 覆盖。[D-03](decisions.md#d-03-损坏重生成收窄)「损坏 ≠ 缺失」语义不变。
...
- **规则来源**：【当前】零内置策略（2026-09-04 定稿）——二进制为纯引擎，默认策略由 init 显式生成的外部 `rules.toml` + `rules.rhai` 提供；**自动生成已移除**（2026-09-12，[D-09](decisions.md#d-09-init-显式生成取代自动生成与全局层路径定稿)），三层皆缺 → 裸兜底 confirm + 提示 init；损坏 ≠ 缺失（告警 + fail-safe confirm 兜底，原文件不动，[D-03](decisions.md#d-03-损坏重生成收窄)）；全局/用户层生成由 init 提供（P8/M8.1 落地）。见 [零内置策略与默认配置生成（定稿）](#零内置策略与默认配置生成定稿)。
