...
- **脚本词汇约定（2026-09-06 定稿，代码落地随 M4.0 前置小改）**：
  - **决策值 = 只读模块常量 `decision::`**：`ALLOW` / `CONFIRM` / `DENY` / `PASS` 四值——无意见也是一种决定，`PASS` 语义为「不表态、交还查表基线」（Rhai 映射空串，Lua 引擎映射 nil）；限定名只读，脚本 `let DENY = "allow"` 之类变量遮蔽污染不了词汇表。引擎同时接受等价裸字符串，最终校验在引擎（双保险）。
  - **ctx 可选字段缺省 = 空字符串**：`ctx.sub` 无子命令时为 `""`，谓词写 `ctx.sub != ""`——不向脚本暴露解释器内部的 unit/nil 语义；「无意见」的契约表述随之引擎无关化。
  - ctx 彻底封装（自定义类型 + 方法化访问）与决策值枚举化（返回值类型收窄为四变体，非法值构造期即报错）**挂 P6**，与 Lua 引擎同批（动的都是 `RuleEngine` 接口层）。
