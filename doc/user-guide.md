...
**Q：`cd /tmp && touch x` 这类「cd 项目外再写」怎么判？**
`cd` 本身放行；行内 `cd` 会切换后续命令写目标的解析基准（段级 cwd 感知）——cd 项目外后的相对写目标判为写项目外 → 确认；子 shell（`(cd /tmp); …`）内的 cd 不影响组外。目标含 `$VAR`/命令替换等静态不可解析形态时保守确认。另：`$( )` 内层命令同样入列裁决，不是免检区。
