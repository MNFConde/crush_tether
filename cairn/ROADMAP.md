...
**Current focus**: P0+P1 已落地（Rust 重写 + 回归用例全绿）；下一步 P2 配置声明层，或按需先做 P4 serve。
...
- [ ] **P2 配置声明层**：`rules.toml` 三层 merge（项目 > 用户 > 全局）+ `[[rules]]` 实例化进 Match 链；`--config`/`CRUSH_TETHER_CONFIG` 覆盖。
  - 验收：三层覆盖/并集/前插语义单测全绿；样例仓库自定义规则改变裁决生效。
- [ ] **P3 脚本层（Rhai 默认）**：`RuleEngine` trait + Rhai 引擎（`max_operations` 限流）+ `rules.rhai` 进管线；`--engine` 切换。
  - 验收：脚本可组合安全原语但不可绕过；死循环被限流兜底。
