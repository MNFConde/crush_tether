...
- [x] **P6 收尾**（M6.1/M6.2 ✅ 2026-09-06；M6.3 待用户确认节奏）：
  ...
  - **M6.3 mdor 侧退役**（Open Questions 1，待用户确认节奏 → **mdor 侧 2026-09-06 执行完毕**：mdor 提交 13d175e——删 crush-guard 目录（含 egg-info/pycache 与 uv.lock 脏改）、`uv tool uninstall` 解除全局安装、`[project.scripts]` 核实已回滚、`.crushrc` 摘除指向已删 guard.py 的**死 hook**（失效模式 #4 实例，bash 回退 Crush 原生确认，留 crush_tether 重挂指引；是否实挂 crush-tether 属新部署决策待用户拍板）、cairn 两篇主题文档 archived + 指针；残留引用仅剩有意的指针与历史记录。M5.3 zcode 实机探针按 2026-09-06 裁定挪至本项一并处理——探针资产就绪（工作区 hook 配置轨 + 插件目录轨），待新 zcode 会话激活后实测，见 M5.3 条）：删 crush-guard 目录、回滚 `[project.scripts]`。
    - 验收：mdor 侧无 crush-guard 残留 ✅；本仓库为唯一实现 ✅；M5.3 实机探针闭环后本项整勾。
