...
- **二跑余波**：CI/claude×2/zcode 三 job 转绿后，crush 双平台首跑到 M8.4 场景组即挂 ask 断言——断言分支写死「三 agent ask 均拒绝」，与脚本头部定性注释（crush=放行）自相矛盾；CI 实际行为符合定性（工具跑了），修断言按 agent 分支（follow-up 提交）。新固化的断言没跑过一次就上库 = 定性与固化物脱节，首跑即暴露。
