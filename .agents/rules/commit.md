...
- **同族坑（条目文档前插吞标题，2026-09-11 两次实证）**：向 LOG 等条目式文档**顶部插入新条目**时，old_string 选了旧条目标题行而 new_string 忘记原样带回——旧标题被静默吞掉、正文变孤儿；一次当场发现、一次隔一轮 audit 才发现（锚点检查不覆盖标题完整性）。对策：前插的 new_string 末尾必须完整包含被匹配的旧标题行；核对手段 = `git show <ref>:<file> | grep '^## '` 与工作区标题序列对照计数。
